...
async def reverse_test(websocket, path):
    ...
    speed_plot = []
    sharktest_list = []
    shark_res, speed_plot = await shark_test(websocket, results, rwnd)
    if not shark_res:
        print("Cannot perform iPerf TCP")
        await websocket.send("Cannot perform iPerf TCP")
        continue
    print("iperf TCP done")
    results += shark_res
    await websocket.send("iPerf TCP done")

    # EFFICIENCY ANALYZER
    await websocket.send("Processing Efficiency")
    efficiency_analyzer_output, eff_rplot = await efficiency_analyzer(websocket)
