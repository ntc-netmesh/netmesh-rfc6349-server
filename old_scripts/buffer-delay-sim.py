...
while (count < packettotal):
	...
	##scan for server
	if ((TCP in pkt1)) and (pkt1[IP].src == src_ip_addr) and (pkt1[TCP].sport == port ):
	...
	if ((TCP in pkt1)) and (pkt1[IP].src == dest_ip_addr and pkt1[TCP].flags == 'A') and (pkt1[TCP].dport == port ):
...
for cnt, i in enumerate(seen):
	...
	if i in tcp_pkt_dict:
		...
		#handle graphing
		temp = time.strftime('%H:%M:%S', time.localtime(end_time/1000))
		if curr_sec == -1:
...
print("ave RTT: " + str(sum(rtt)/len(rtt)) + " ms")
